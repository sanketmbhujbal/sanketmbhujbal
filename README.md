<!-- ## Hi there 👋 -->

<!--
**sanketmbhujbal/sanketmbhujbal** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
<h1 align="center">Hi there! 👋</h1>
<p align="center">



## About Me

I'm a **Data Scientist / Machine Learning Engineer** with hands-on experience building **end-to-end ML systems** — from data ingestion and modeling to deployment and business impact.

💼 **What I do**

- Apply **statistics, machine learning, and AI** to solve real-world business problems  
- Build **scalable pipelines**, predictive models, and decision-support systems  
- Translate complex analyses into **clear, actionable insights** for stakeholders  

📈 **Impact highlights**

- Improved model-driven outcomes such as **CTR, conversion, retention, and revenue**
- Designed and productionized **recommender systems, forecasting models, and analytics dashboards**

🚀 **Currently seeking**
Full-time roles in **Machine Learning, Data Science, or AI Engineering**

🔗 **Explore my work**
👉 My Repositories showcase **real-world, end-to-end projects**

[Check out my Portfolio!](https://sanket-bhujbal-nn01mi4.gamma.site/)

<br><br>

---

## 🛠️ Skills & Tooling

### Machine Learning & AI
- **Classical ML**: Scikit-learn, XGBoost, Stats-based modeling  
- **Deep Learning**: PyTorch, TensorFlow, Keras  
- **NLP & LLMs**: HuggingFace, LangChain, spaCy, NLTK  
- **Generative AI**: RAG pipelines, prompt engineering, LLM-based agents  

![PyTorch](https://img.shields.io/badge/PyTorch-logo?logo=pytorch&logoColor=white&color=red)
![TensorFlow](https://img.shields.io/badge/TensorFlow-logo?logo=tensorflow&logoColor=white&color=orange)
![Scikit-Learn](https://img.shields.io/badge/scikit--learn-logo?logo=scikit-learn&logoColor=white&color=blue)
![HuggingFace](https://img.shields.io/badge/Transformers-logo?logo=hugging%20face&logoColor=white&color=yellow)
![LangChain](https://img.shields.io/badge/LangChain-blue?logo=langchain&logoColor=white)

---

### Data Analysis & Visualization
- **Data wrangling & EDA**: Pandas, NumPy, SciPy  
- **Visualization**: Matplotlib, Seaborn, Plotly  
- **Dashboards**: Power BI, Tableau  

![Pandas](https://img.shields.io/badge/Pandas-logo?logo=pandas&logoColor=white&color=blue)
![NumPy](https://img.shields.io/badge/NumPy-logo?logo=numpy&logoColor=white&color=blue)
![Plotly](https://img.shields.io/badge/Plotly-3F4F75?logo=plotly&logoColor=white)

---

### Data Engineering & Databases
- **SQL & NoSQL**: PostgreSQL, MySQL, MongoDB  
- **Big Data**: PySpark, Databricks  
- **Vector Databases**: ChromaDB  

![PostgreSQL](https://img.shields.io/badge/PostgreSQL-logo?logo=postgresql&logoColor=white&color=blue)
![MongoDB](https://img.shields.io/badge/MongoDB-logo?logo=mongodb&logoColor=white&color=green)
![PySpark](https://img.shields.io/badge/PySpark-e25a1c?logo=apache-spark&logoColor=white)

---

### MLOps & Cloud
- **Cloud**: AWS  
- **Orchestration**: Airflow  
- **Containers & CI**: Docker, Git  
- **Warehousing**: Snowflake  

![AWS](https://img.shields.io/badge/AWS-logo?logo=amazonwebservices&logoColor=white&color=yellow)
![Docker](https://img.shields.io/badge/Docker-logo?logo=docker&logoColor=white&color=blue)
![Airflow](https://img.shields.io/badge/Airflow-logo?logo=apache-airflow&logoColor=white&color=orange)

---

### Application Development
- **Backend & APIs**: Flask  
- **ML Apps & Prototypes**: Streamlit  
- **Web**: HTML, CSS, JavaScript  

![Streamlit](https://img.shields.io/badge/streamlit-logo?logo=streamlit&logoColor=white&color=orange)
![Flask](https://img.shields.io/badge/Flask-logo?logo=flask&logoColor=white&color=black)

---

### Programming & Systems
- **Languages**: Python, SQL, C, C++, JavaScript  
- **OS**: Linux, Windows  

![Python](https://img.shields.io/badge/Python-logo?logo=python&logoColor=white&color=yellow)
![Linux](https://img.shields.io/badge/Linux-logo?logo=linux&logoColor=white&color=purple)

---

<br><br>
## 🤝 Connect with me:
<p align="left">
	<a href="mailto:sanketmbhujbal99@gmail.com"><img img src="https://img.shields.io/badge/Email-%23EA4335.svg?style=plastic&logo=Email&logoColor=white" alt="Gmail"/></a>
	<a href="https://www.linkedin.com/in/sanketmbhujbal/"><img src="https://img.shields.io/badge/LinkedIn-%230A66C2.svg?style=plastic&logo=LinkedIn&logoColor=white" alt="LinkedIn"/></a>
</p>
</p>
